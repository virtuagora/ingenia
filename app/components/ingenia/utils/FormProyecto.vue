  ...
  <div>
    <div class="field">
      <label class="label is-size-4" :class="{'has-text-danger': errors.has('project.name')}">
        <i class="fas fa-angle-double-right"></i> Nombre del proyecto *
      </label>
      <div class="control">
        <input
    ...
    <div class="field">
      <label class="label is-size-4" :class="{'has-text-danger': errors.has('project.abstract')}">
        <i class="fas fa-angle-double-right"></i> Resumen del proyecto *
      </label>
    ...
    <div class="field">
      ...
      <div class="control">
        ...
        <span v-show="errors.has('project.foundation')" class="help is-danger">
          <i class="fas fa-times-circle fa-fw"></i>
          &nbsp;{{errors.first('project.foundation')}}
        </span>
      </div>
    </div>
    <div class="field">
      <label
        ...
        :class="{'has-text-danger': errors.has('project.category_id')}"
      >
        <i class="fas fa-angle-double-right"></i>Seleccione la categoria del proyecto *
      </label>
      ...
      <div class="notification is-primary" v-show="project.category_id != null">
        <p
          v-for="categoria in categorias"
          :key="categoria.id"
          v-show="project.category_id == categoria.id"
    ...
    <div class="field">
      <label class="label is-size-4" :class="{'has-text-danger': errors.has('enEjecucion')}">
        <i class="fas fa-angle-double-right"></i> ¿El proyecto ya esta en ejecución o en funcionamiento? *
      </label>
      <p>
    ...
    <div class="field" v-if="enEjecucion">
      <label
        ...
        :class="{'has-text-danger': errors.has('project.previous_work')}"
      >
        <i class="fas fa-caret-right"></i> ¿De qué forma, cómo y dónde se está ejecutando?
      </label>
      <div class="control">
        <b-input
          type="textarea"
          v-model="project.previous_work"
        ...
        <span v-show="errors.has('project.previous_work')" class="help is-danger">
          <i class="fas fa-times-circle fa-fw"></i>
          &nbsp;{{errors.first('project.previous_work')}}
        </span>
      </div>
    </div>
    ...
    <div class="field">
      <label
        class="label is-size-4"
        :class="{'has-text-danger': errors.has('project.neighbourhoods')}"
      >
        <i class="fas fa-angle-double-right"></i>¿En que barrio/s se llevará adelante? *
      </label>
      <p>Indiquen el/los barrios donde realizarán el proyecto. Separe con comas, o presione TAB</p>
      <br>
      <b-taginput
        v-model="project.neighbourhoods"
      ...
      <span v-show="errors.has('project.neighbourhoods')" class="help is-danger">
        <i class="fas fa-times-circle fa-fw"></i>
        &nbsp;{{errors.first('project.neighbourhoods')}}
      </span>
      <br>
    </div>
    <div class="field">
      <label class="label is-size-4" :class="{'has-text-danger': errors.has('project.goals')}">
        <i class="fas fa-angle-double-right"></i> Objetivos *
      </label>
      ...
      <div class="control">
        ...
        <div class="field is-grouped">
          ...
          <p class="control">
            <b-tooltip
              :label="disableAddObjetivo ? 'Falta información' : 'Agregar!'"
    ...
    <div class="field">
      <label class="label is-size-4" :class="{'has-text-danger': errors.has('project.schedule')}">
        <i class="fas fa-angle-double-right"></i> Calendario de Actividades *
      </label>
      ...
      <div class="field is-grouped">
        <div class="control">
          <b-datepicker
            ...
            v-model="dateActividad"
            :mobile-native="false"
            size="is-medium"
            :date-formatter="(date) => date.toLocaleDateString('es-AR')"
            :min-date="new Date()"
            :max-date="new Date('04/30/2020')"
            icon="calendar-alt"
          ></b-datepicker>
        </div>
        <p class="control is-expanded">
        ...
        <p class="control">
          <b-tooltip
            :label="disableAddActividad ? 'Falta información' : 'Agregar!'"
    ...
    <div class="field">
      ...
      <label class="label is-size-4" v-if="project.budget.length">
        ...
        Monto total solicitado: $ {{montoTotal}}
      </label>
    </div>
    <hr>
    <div class="field">
      <label class="label is-size-4" :class="{'has-text-danger': errors.has('conOrganizacion')}">
        <i class="fas fa-angle-double-right"></i> ¿Las actividades las realizarán en coordinación con otras organizaciones y/o instituciones? *
      </label>
      <p>
        Se refiere a organizaciones y/o instituciones
        <u>diferentes</u> a la que pertenece el equipo
      </p>
      <p>
        <b>IMPORTANTE:</b> Si el proyecto se realiza en coordinación con otra institución y/o organización, debera adjuntar la carta aval en el sistema en otro momento.
      </p>
      <br>
      <b-field>
        <b-radio-button
          v-model="conOrganizacion"
      ...
      <span v-show="errors.has('conOrganizacion')" class="help is-danger">
        <i class="fas fa-times-circle fa-fw"></i>
        &nbsp;{{errors.first('conOrganizacion')}}
      </span>
    </div>
    <br>
    <form-organizacion
      ref="formOrganizacion"
      v-if="conOrganizacion"
      :organization.sync="project.organization"
    ></form-organizacion>
  </div>
</template>
